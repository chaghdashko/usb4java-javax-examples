...
public class Adb {
	...
	private static void checkDevice(UsbDevice usbDevice, List<AdbDevice> adbDevices) {
		...
		for (UsbInterface iface : ifaces) {
			UsbInterfaceDescriptor usbInterfaceDescriptor = iface.getUsbInterfaceDescriptor();
			System.out.println(usbInterfaceDescriptor.bInterfaceClass() + ", " + usbInterfaceDescriptor.bInterfaceSubClass() + ", " + usbInterfaceDescriptor.bInterfaceProtocol());
			...
			if (((ed1.bmAttributes() & UsbConst.ENDPOINT_TYPE_BULK) == 0) || ((ed2.bmAttributes() & UsbConst.ENDPOINT_TYPE_BULK) == 0))
				continue;
	...
	private static boolean isAdbInterface(UsbInterface iface) {
		...
		return desc.bInterfaceClass() == ADB_CLASS && desc.bInterfaceSubClass() == ADB_SUBCLASS && desc.bInterfaceProtocol() == ADB_PROTOCOL;
	}
